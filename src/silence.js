(function($) {
  ...
  class Silence {
    ...
    buildCustomElements() {
      // Change page title.
      const blogTitle = $(this.cnblogs.blogTitle)
        .find("h1 a")
        .html();
      const autherName = $(this.cnblogs.publicProfile)
        .find("a:eq(0)")
        .html();
      ...
      $navList
        ...
        .eq(1)
        .after(
          `<li><a id="blog_nav_tags" class="menu" href="https://www.cnblogs.com/${currentBlogApp}/tag">标签</a></li>`
        );
    ...
    buildPostSponsor() {
      ...
      $("#blog_post_info").prepend(`
            <div class="esa-sponsor">
                <a class="github" href="${
                  github.enable ? github.link : "https://github.com/Kaiyuan/donate-page"
    ...
    buildPostCodeCopyBtns() {
      ...
      $.each($pres, (index, pre) => {
        $(pre)
          .find("code")
          .attr("id", `copy_target_${index}`);
        $(pre).prepend(
          `<div data-tips="复制代码" class="esa-clipboard-button" data-clipboard-target="#copy_target_${index}">Copy</div>`
        );
      });
